...
namespace VisionDesigner
{
    ...
    */
    ...
    {
        MVD_EDGEINSP_FLAW_TYPE_PAIR_WIDTH = 0x0001, // 边缘对宽度缺陷
        MVD_EDGEINSP_FLAW_TYPE_PAIR_OFFSET = 0x0002, // 边缘对位置偏移
        MVD_EDGEINSP_FLAW_TYPE_PAIR_ROUGH = 0x0003, // 边缘对阶梯缺陷
        MVD_EDGEINSP_FLAW_TYPE_PAIR_CRACK = 0x0004, // 边缘对断裂缺陷
        MVD_EDGEINSP_FLAW_TYPE_PAIR_BUBBLE = 0x0005, // 边缘对气泡缺陷


		MVD_EDGEINSP_FLAW_TYPE_EDGE_OFFSET              = 0x0006, // 边缘位置偏移
		MVD_EDGEINSP_FLAW_TYPE_EDGE_ROUGH               = 0x0007, // 边缘凸起或凹坑缺陷
		MVD_EDGEINSP_FLAW_TYPE_EDGE_CRACK               = 0x0008, // 边缘断裂缺陷
		MVD_EDGEINSP_FLAW_TYPE_EDGE_GRAD                = 0x0009, // 边缘阶梯差缺陷

		//边缘对模型缺陷检测新增细分缺陷
		MVD_EDGEINSP_FLAW_TYPE_PAIR_NARROW = 0x000A,  // 窄缺陷：（宽度过小）
		MVD_EDGEINSP_FLAW_TYPE_PAIR_WIDE   = 0x000B,  // 宽缺陷：（宽度过大）
		MVD_EDGEINSP_FLAW_TYPE_PAIR_GRAY   = 0x000C,  // 亮度缺陷（暂未启用，若亮度细分使能开启，则分为亮缺陷、暗缺陷）
		MVD_EDGEINSP_FLAW_TYPE_PAIR_BRIGHT = 0x000D,  // 亮缺陷：（灰度过大）
		MVD_EDGEINSP_FLAW_TYPE_PAIR_DARK   = 0x000E,  // 暗缺陷：（灰度过小）
		MVD_EDGEINSP_FLAW_TYPE_PAIR_GRAY_STEP = 0x000F, //亮度不均缺陷
		MVD_EDGEINSP_FLAW_TYPE_PAIR_CONNECT   = 0x0010, // 边缘对连接缺陷（即模型是断裂，实测为OK）

		MVD_EDGEINSP_FLAW_TYPE_PAIR_SIDE_CRACK  = 0x0011, // ROI端部 断裂缺陷
		MVD_EDGEINSP_FLAW_TYPE_PAIR_SIDE_OFFSET = 0x0012, // ROI端部 位置偏移
		MVD_EDGEINSP_FLAW_TYPE_PAIR_SIDE_NARROW = 0x0013, // ROI端部 窄缺陷：（宽度过小）
		MVD_EDGEINSP_FLAW_TYPE_PAIR_SIDE_WIDE   = 0x0014, // ROI端部 宽缺陷：（宽度过大）
		MVD_EDGEINSP_FLAW_TYPE_PAIR_SIDE_BRIGHT = 0x0015, // ROI端部 亮缺陷：（灰度过大）
		MVD_EDGEINSP_FLAW_TYPE_PAIR_SIDE_DARK   = 0x0016, // ROI端部 暗缺陷：（灰度过小）

		MVD_EDGEINSP_FLAW_TYPE_PAIR_ONESIDE_CRACK = 0x0017, // 单边断裂缺陷（即区域一侧可以找到点）
		MVD_EDGEINSP_FLAW_TYPE_PAIR_SIDE_ONESIDE_CRACK = 0x0018, // ROI端部 单边断裂缺陷（即区域一侧可以找到点）
	...
	enum _MVD_EDGE_FIND_TYPE__
	{
		MVD_EDGE_WIDEST_TYPE = 1, // 查找最宽边缘
		MVD_EDGE_NEAREST_TYPE = 2, // 查找最接近边缘
		MVD_EDGE_FIRST_STRONGEST_TYPE  = 3, // 查找最强最接近边缘
	};
	...
	*/
	...
	enum _MVD_EDGEINSP_PAIR_MOD_PRO_TYPE__
	{
		MVD_EDGEINSP_PAIR_FIND_MOD_PRO_TYPE_BUILD  = 0, // 首次建模
		MVD_EDGEINSP_PAIR_FIND_MOD_PRO_TYPE_LEARN  = 1, // 缺陷学习
		MVD_EDGEINSP_PAIR_FIND_MOD_PRO_TYPE_QUERY  = 2, // 模型查询
		MVD_EDGEINSP_PAIR_FIND_MOD_PRO_TYPE_MODIFY = 3, // 手动修改
	};
	enum _MVD_EDGEINSP_PAIR_MOD_FEATURE_TYPE__
	{
		MVD_EDGEINSP_PAIR_FIND_MOD_FEATURE_TYPE_WIDTH   = 1, // 宽度
		MVD_EDGEINSP_PAIR_FIND_MOD_FEATURE_TYPE_GRAY    = 2, // 灰度
		MVD_EDGEINSP_PAIR_FIND_MOD_FEATURE_TYPE_POSTION = 3, // 位置
	};
	...
	*/
    ...
    /************************************************************************/
	// 缺陷类型(输出使用)
	enum _MVD_SOLARCELL_INSPECTION_FLAW_TYPE__
	{
		MVD_SOLARCELL_INSPECTION_FLAW_TYPE_STRING_LOC_FAIL   = 0x0001, // 串定位失败
		MVD_SOLARCELL_INSPECTION_FLAW_TYPE_STRING_DIST_ERROR = 0x0002, // 串间距异常
		MVD_SOLARCELL_INSPECTION_FLAW_TYPE_LINE_ERROR        = 0x0003, // 抓边异常

		//正面检缺陷类型
		MVD_SOLARCELL_PI_FLAW_TYPE_ABNORMAL                  = 0x1000,  // 片异常
		MVD_SOLARCELL_PI_FLAW_TYPE_EDGE_CHIPPINNG            = 0x1001,  // 崩边缺陷
		MVD_SOLARCELL_PI_FLAW_TYPE_PARALLEL_GAP              = 0x1002,  // 片间距异常缺陷
		MVD_SOLARCELL_PI_FLAW_TYPE_STR_GAP                   = 0x1003,  // 串间距异常缺陷
		MVD_SOLARCELL_PI_FLAW_TYPE_GRID_BIAS                 = 0x1004,  // 主栅偏移(露白)	
		MVD_SOLARCELL_PI_FLAW_TYPE_FOREIGN                   = 0x1005,  // 脏污，异物，划痕缺陷
		MVD_SOLARCELL_PI_FLAW_TYPE_SHORT_CIRCUIT             = 0x1006,  // 正面短路
		MVD_SOLARCELL_PI_FLAW_TYPE_LOST_GRID                 = 0x1007,  // 少条
		MVD_SOLARCELL_PI_FLAW_TYPE_FLAT_NG                   = 0x1008,  // 拍平缺陷
		MVD_SOLARCELL_PI_FLAW_TYPE_THREE_U                   = 0x1009,  // 正面三段头
		MVD_SOLARCELL_PI_FLAW_TYPE_FLAT_STAND_NG             = 0x1010,  // 拍平立起缺陷
		MVD_SOLARCELL_PI_FLAW_TYPE_FLAT_OK                   = 0x1011,  // 正常拍平
		MVD_SOLARCELL_PI_FLAW_TYPE_GRID_BACKWARD             = 0x1012,  // 焊带后溜
		MVD_SOLARCELL_PI_FLAW_TYPE_DIRTY_LINE                = 0x1013,  // 线性异物
		MVD_SOLARCELL_PI_FLAW_TYPE_DIRTY_CHIP                = 0x1014,  // 片状异物
		MVD_SOLARCELL_PI_FLAW_TYPE_DIRTY_CRYSTAL             = 0x1015,  // 结晶异物

		//背面缺陷检测类型
		MVD_SOLARCELL_NI_FLAW_TYPE_WELD_BLOCK_ERROR          = 0x2000,  // 焊接块异常（露白）
		MVD_SOLARCELL_NI_FLAW_TYPE_SHORT_CIRCUIT             = 0x2001,  // 背面短路
		MVD_SOLARCELL_NI_FLAW_TYPE_ABNORMAL                  = 0x2002,  // 片异常
		MVD_SOLARCELL_NI_FLAW_TYPE_FORWARD_SLIP              = 0x2003,  // （焊带）前溜
	};
    ...
    */
    enum _MVD_DL_SDK_TYPE__
    {
        ...
        MVD_DL_SDK_TYPE_MAX    = 82,           /// 深度SDK类型最大值		
    };
    /************************************************************************/
    /*                            深度算子类                            */
    /************************************************************************/
	...
	*  @brief 图像分割类模型类型,
	*/
	typedef enum  MVD_CNNSEGMENT_MODEL_TYPE_
	{
		MVD_CNNSEGMENT_MODEL_TYPE_UNCERTAIN = 0,    // 不确定模型
		MVD_CNNSEGMENT_MODEL_TYPE_SUPERVISE = 1,     // 单图像分析模式，运行参数SampleInterval，ImgPadType，AlgRunMode仅在此模型类型下生效
		MVD_CNNSEGMENT_MODEL_TYPE_COMPARE = 2,       // 图像对比模式
		MVD_CNNSEGMENT_MODEL_TYPE_TEMPLATE = 3,      // 自学习模板对比模式
		MVD_CNNSEGMENT_MODEL_TYPE_SEG = 4,           // 单图像分割模式
		MVD_CNNSEGMENT_MODEL_TYPE_SEMANTIC_SEG = 5   // 机器人分割模式    

	} MVD_CNNSEGMENT_MODEL_TYPE;
	...
	*/
	...
	typedef struct _MVD_EDGEINSP_EDGE_PAIR_MOD_OPEN_INFO__
	{
		float    feature_min;    // 特征低值
		float    feature_max;    // 特征高值
		MVD_BOX_F caliper_box;    // 卡尺ROI
		int caliper_loc_id; // 卡尺局部ID（在当前查找ROI中的ID序号）
		int pts_sts;        // 边缘查找状态
	};
