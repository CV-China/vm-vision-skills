...
namespace VisionDesigner
{
    ...
    */
	...
	//拼接两张图存在的重合匹配点数据
	struct _MVD_OPTSTITCH_STITCH_MATCH_DATA__
	{
		MVD_POINT_F *points[2];    // 两张图匹配重合的点集
		int      img_index[2]; // 两张图对应的图像数据编号
		int      points_num;   // 两张图匹配重合点数
	};
   ...
   /** @enum  _MVD_OCR_CONSTRAINT_RULE_TYPE__
   *  @brief  字符约束规则类型
   */
   typedef enum _MVD_OCR_CONSTRAINT_RULE_TYPE__
   {
	   MVD_OCR_CONSTRAINT_RULE_BY_CHAR = 0,  // 指定字符约束
	   MVD_OCR_CONSTRAINT_RULE_BY_POS = 1    // 指定位置约束
   }MVD_OCR_CONSTRAINT_RULE_TYPE;

   /** @enum  _MVD_OCR_CONSTRAINT_CHAR__
   *  @brief 约束字符，使用当前系统默认编码，支持中英文
   */
   typedef struct _MVD_OCR_CONSTRAINT_CHAR__
   {
	   char  character[4];  // 约束字符，以'\0'结尾的字符串表示；实际只取该字符串中的第一个字符作为约束字符
   }MVD_OCR_CONSTRAINT_CHAR;

   /** @enum  _MVDOCR_CONSTRAINT_BY_CHAR_PARAM__
   *  @brief 指定字符约束规则
   */
   typedef struct _MVDOCR_CONSTRAINT_BY_CHAR_PARAM__
   {
	   MVD_OCR_CONSTRAINT_CHAR srcChar;  // 指定约束源字符	   
	   MVD_OCR_CONSTRAINT_CHAR dstChar;  // 指定约束目标字符，与源字符数组一一对应 
	   int nReserved[4];                 // 保留字节
   }MVD_OCR_CONSTRAINT_BY_CHAR_PARAM;

   /** @enum  _MVD_OCR_CONSTRAINT_BY_POS_PARAM__
   *  @brief 指定位置约束规则
   */
   typedef struct _MVD_OCR_CONSTRAINT_BY_POS_PARAM__
   {
	   int nPosIndex;                    // 指定约束字符索引位置，从0开始；0表示第1个位置，1表示第2个位置
	   MVD_OCR_CONSTRAINT_CHAR dstChar;  // 指定约束字符索引位置的替换字符
	   int nReserved[4];                 // 保留字节
   }MVD_OCR_CONSTRAINT_BY_POS_PARAM;

   /** @enum  _MVD_OCR_CONSTRAINT_RULE_ITEM__
   *  @brief 指定字符约束，若识别结果中的字符与约束源字符数组中的指定值一样，则替换为约束目标字符中的字符
   *  @brief 指定位置约束，指定的索引位置将被替换为目标字符
   */
   typedef  struct _MVD_OCR_CONSTRAINT_RULE_ITEM__
   {
	   MVD_OCR_CONSTRAINT_RULE_TYPE nConstraintType;   // 约束类型
	   MVD_OCR_CONSTRAINT_BY_CHAR_PARAM stByCharParam; // 按字符内容进行替换，当nConstraintType为MVD_OCR_CONSTRAINT_RULE_BY_CHAR时有效
	   MVD_OCR_CONSTRAINT_BY_POS_PARAM  stByPosParam;  // 按字位位置进行替换，当nConstraintType为MVD_OCR_CONSTRAINT_RULE_BY_POS时有效
	   int  nReserved[16];                             // 保留字节
   }MVD_OCR_CONSTRAINT_RULE_ITEM;

   /** @enum  _MVD_OCR_CONSTRAINT_PARAM__
   *  @brief 字符识别ROI字符替换参数链表
   *  @brief 替换规则按顺序依次执行，已被替换字符不再被其他规则替换
   */
   typedef  struct _MVD_OCR_CONSTRAINT_PARAM__
   {
	   MVD_OCR_CONSTRAINT_RULE_ITEM stConstaintItem[100]; // 字符约束规则
	   int  nConstraintNum;                               // 实际字符约束规则数量
	   int  nReserved[15];                                // 保留字节
   }MVD_OCR_CONSTRAINT_PARAM;
   ...
   /** @enum  _MVD_OCR_ROI_ASSO_DATA__
   *  @brief 字符识别ROI关联数据，包含过滤数据和字符约束数据
   */
   typedef struct _MVD_OCR_ASSOCIATED_DATA_
   {
	   MVD_OCR_FILTER_LIST stROIFilter;
	   int                 nReserved1;              // 保留字节1
	   MVD_OCR_CONSTRAINT_PARAM stConstraintParam;  // 字符约束参数
	   int                 nReserved2[32];          // 保留字节2
   }MVD_OCR_ASSOCIATED_DATA;
    ...
    */
    ...
    enum _MVD_EDGEINSP_FLAW_DEFECT_TYPE__
    {
		...
		MVD_EDGEINSP_FLAW_DEFECT_TYPE_NARROW     = 7,  // 窄（宽度过小）缺陷
		MVD_EDGEINSP_FLAW_DEFECT_TYPE_WIDE       = 8,  // 宽（宽度过大）缺陷
		MVD_EDGEINSP_FLAW_DEFECT_TYPE_BRIGHT     = 9,  // 亮（灰度过大）缺陷
		MVD_EDGEINSP_FLAW_DEFECT_TYPE_DARK       = 10, // 暗（灰度过小）缺陷
		MVD_EDGEINSP_FLAW_DEFECT_TYPE_POS_OFFSET = 11, // 位置偏移缺陷
		MVD_EDGEINSP_FLAW_DEFECT_TYPE_LINES      = 12, // 基准直线对异常
		MVD_EDGEINSP_FLAW_DEFECT_TYPE_DIST       = 13, // 宽度缺陷（若宽度细分使能开启，则分为窄缺陷、宽缺陷）
		MVD_EDGEINSP_FLAW_DEFECT_TYPE_GRAY       = 14, // 亮度缺陷（若亮度细分使能开启，则分为亮缺陷、暗缺陷）

		MVD_EDGEINSP_FLAW_DEFECT_TYPE_GRAY_STEP  = 15, // 亮度不均缺陷
		MVD_EDGEINSP_FLAW_DEFECT_TYPE_GRAY_MOTTLE = 16, // 灰度斑驳缺陷（观察对象：边缘点对间）
    };
	
		 /** @enum  _MVD_EDGEINSP_EDGE_FLAW_TYPE__
     *  @brief Type of the flaw of the edge inspection
     */
    enum _MVD_EDGE_POS_TREND_FLAW_TYPE_
    {
		MVD_EDGEINSP_TREND_EDGE_FLAW_TYPE_CHIPPING    = 0x00000001,  // 崩边
		MVD_EDGEINSP_TREND_EDGE_FLAW_TYPE_DISLOCATION = 0x00000002,  // 错位
    };
    ...
    */
	...
	/** @struct  _MVD_GRAY_STATISTIC_INFO__
	*  @brief LineEdgePairFlawInsp gray statistic info
	*/
	struct _MVD_GRAY_STATISTIC_INFO__
	{
		float  fMinValue;        // 最小值
		float  fMaxValue;        // 最大值
		float  fMaxMinValue;     // 极差（或全距）
		float  fMeanValue;       // 均值
		float  fVarValue;        // 方差
		float  fStdValue;        // 标准差
		float  fCvValue;         // 变异系数（标准差 / 均值）
		float  fContrast;        // 对比度
	};
	...
	*  @brief 颜色范围调节模式
	*/
	typedef enum _MVD_COLOR_RANGE_ADJUST_TYPE__
	{
		MVD_COLOR_RANGE_ADJUST_ADD = 0x00000001,		    ///> 合并
		MVD_COLOR_RANGE_ADJUST_SUB = 0x00000002,		    ///> 减除
		MVD_COLOR_RANGE_ADJUST_CANCEL = 0x00000003,         ///> 撤销
	}MVD_COLOR_RANGE_ADJUST_TYPE;
    ...
    */
    enum _MVD_DL_SDK_TYPE__
    {
		...
		MVD_DL_InstanceSegmentTR = 87,           /// 实例分割TR
        MVD_DL_SDK_TYPE_MAX    = 87,           /// 深度SDK类型最大值
    };
    /************************************************************************/
    /*                            深度算子类                            */
    /************************************************************************/
    ...
    */
    enum _MVD_CNN_MODEL_TYPE__
    {
		...
		MVD_CNN_MODEL_TYPE_CHANGE_DETECT           = 0x0015, // 变换检测
		MVD_CNN_MODEL_TYPE_MULTI_SOURCE_DETECT     = 0x0016, // 多源检测
    };
	...
	*/
	typedef enum _MVD_CNN_PREDICT_MODE__
	{
        ...
        MVD_CNN_PREDICT_BLOB_NORMAL = 0x00000003, ///> 图像分割V3，按ROI管控 普通模式
        MVD_CNN_PREDICT_BLOB_SOD = 0x00000004     ///> 图像分割V3，按ROI管控 SOD模式
    }MVD_CNN_PREDICT_MODE;

    /** @enum  _MVD_CNN_SOD_TYPE__
    *  @brief SOD模式
    */
    typedef enum _MVD_CNN_PREDICT_SOD_MODE__
    {
        MVD_CNN_PREDICT_SOD_MAN_SLIDING = 0x00000000,	 ///> 手动滑窗，外部仅配置滑窗参数即可
        MVD_CNN_PREDICT_SOD_AUTO_SLIDING = 0x00000001	 ///> 自适应滑窗，外部仅配置窗口分辨率即可
    }MVD_CNN_PREDICT_SOD_MODE;
	...
	*/
    ...
    {
        // 滑窗参数，SODType为MVD_CNN_PREDICT_SOD_MAN_SLIDING模式时，配置生效
		unsigned int nCol;                        // x方向滑窗数
		unsigned int nRow;                        // y方向滑窗数
        float        fOverlap;                    // xy滑窗重叠率

        // 滑窗分辨率，SODType为MVD_CNN_PREDICT_SOD_AUTO_SLIDING模式时，配置生效(目前仅支持图像分割算子)
        unsigned int nAdaptiveWindowsWidth;       // 自适应窗口宽度，默认值：模型分辨率宽度
        unsigned int nAdaptiveWindowsHeight;      // 自适应窗口高度，默认值：模型分辨率高度

        MVD_CNN_PREDICT_SOD_MODE SODType;         // SOD模式，默认值：MVD_CNN_PREDICT_SOD_MAN_SLIDING
	...
	*/
    ...
    // 轮廓参数
    typedef struct _MVD_CONTOUR_S_
    {
        MVD_POINT_S* pPoint;    // 轮廓点集
        signed int   nPointNum; // 轮廓点数
    }MVD_CONTOUR_S;

	/** @enum  _MVD_CNN_REGION_TYPE_
	*  @brief 二值化类型
	*/
	enum MVD_THRESHOLD_TYPE
	{
		MVD_THRESHOLD_TYPE_BINARY = 1,	       // 不进行二值化
		MVD_THRESHOLD_TYPE_SINGLE = 2,	       // 单阈值
		MVD_THRESHOLD_TYPE_DOUBLE = 3,	       // 双阈值
		MVD_THRESHOLD_TYPE_AUTO = 4,		   // 自动阈值
		MVD_THRESHOLD_TYPE_SOFT_FIEXED = 5,    // 软阈值（固定）
		MVD_THRESHOLD_TYPE_SOFT_RELATIVE = 6   // 软阈值（相对）
	};

	enum MVD_POLARITY_TYPE
	{
		MVD_POLARITY_TYPE_Bright = 0,  // 目标物体亮于背景
		MVD_POLARITY_TYPE_Dark = 1	 // 目标物体暗于背景
	};

	// 连通性
	enum MVD_CONNECTIVITY_TYPE
	{
		MVD_CONNECTIVITY_TYPE_4 = 4,  // 4连通
		MVD_CONNECTIVITY_TYPE_8 = 8   // 8连通
	};
	...
	enum MVD_SORT_TYPE
	{	
		MVD_SORT_TYPE_ASC = 1,   // 升序
		MVD_SORT_TYPE_DESC = 2,  // 降序
		MVD_SORT_TYPE_NONE = 3   // 不排序
	};

	// 排序特征
	enum MVD_FEATURE_TYPE
	{
		MVD_FEATURE_TYPE_AREA = 1,		     // 面积
		MVD_FEATURE_TYPE_PERIMETER = 2,	     // 周长
		MVD_FEATURE_TYPE_CIRCULARITY = 3,     // 圆形度
		MVD_FEATURE_TYPE_RECTANGULARITY = 4,  // 矩形度
		MVD_FEATURE_TYPE_CENTROIDX = 5,	     // 连通域中心X
		MVD_FEATURE_TYPE_CENTROIDY = 6,	     // 连通域中心Y
		MVD_FEATURE_TYPE_BOXANGLE = 7,	     // 外接非0度矩形角度
		MVD_FEATURE_TYPE_BOXWIDTH = 8,	     // 外接非0度矩形宽
		MVD_FEATURE_TYPE_BOXHEIGHT = 9,	     // 外接非0度矩形高
		MVD_FEATURE_TYPE_RECTX = 10,		     // 外接0度矩形左上顶点X
		MVD_FEATURE_TYPE_RECTY = 11,		     // 外接0度矩形左上顶点Y
		MVD_FEATURE_TYPE_AXISANGLE = 12,	     // 二阶中心距主轴角度
		MVD_FEATURE_TYPE_AXISRATIO = 13	     // 轴比，外接非0度矩形（短轴/长轴）
	};

	// 轮廓类型
	enum MVD_CONTOUR_TYPE
	{
		MVD_CONTOUR_TYPE_NONE = 0,		   // 不输出轮廓
		MVD_CONTOUR_TYPE_NONSEQUENTIAL = 1,  // 输出无序轮廓
	};

    /* @struct _MVDCNN_IMAGESEGMENT_CLASS_PARAM_
    * @brief 图像分割按类别索引配置过滤参数结构体
    *        单独通过SetClassParam配置指定索引的类别参数，类别图结果仅检出nIsClassParamValid为MVD_True的类别。
    */
    typedef struct _MVDCNN_IMAGESEGMENT_CLASS_PARAM_
    {
        int nIsOutput;           // 类别使能，默认值：MVD_FALSE
        // 设置为MVD_TRUE时，以下参数配置有效，类别图中检出该类别

        int nMinScore;           // 最小分数，范围：[0, 255]，默认值：60

        int nIsBlobValid;        // Blob使能，默认值：MVD_FALSE
        // 设置为MVD_TRUE时，Blob参数配置有效，结果输出Blob信息

        // Blob参数
        int nMaxFindNum; 	     // 最大查找个数，范围：[0, 1000]，默认值：100
        int nHoleMinArea; 	     // 孔洞最小面积，范围：[0, 999999999]，默认值：0

        int nAreaEnable; 	     // 面积筛选，默认值：MVD_FALSE
        int nMinArea;		     // 面积下限，范围：[1, 999999999]，默认值：10
        int nMaxArea;            // 面积上限，范围：[1, 999999999]，默认值：999999999

        int nShortAxisEnable;    // 短轴筛选，默认值：MVD_FALSE
        int nMinShortAxis;       // 短轴下限，范围：[1, 999999999]，默认值：1
        int nMaxShortAxis;       // 短轴上限，范围：[1, 999999999]，默认值：999999999

        int nLongAxisEnable;     // 长轴筛选，默认值：MVD_FALSE
        int nMinLongAxis;        // 长轴下限，范围：[1, 999999999]，默认值：10
        int nMaxLongAxis;        // 长轴上限，范围：[1, 999999999]，默认值：999999999

        int nReserved[50];       // 保留
    }MVDCNN_IMAGESEGMENT_CLASS_PARAM;

	/** @struct _MVD_MULTI_ROI_BLOB_PARAM__
	*  @brief  多ROI blob分析ROI参数
	*/
	struct _MVD_MULTI_ROI_BLOB_PARAM__
	{
		// blob分析二值化参数
		MVD_THRESHOLD_TYPE emThresholdType;        // 阈值方式，默认：MVD_THRESHOLD_TYPE_SINGLE
		MVD_POLARITY_TYPE emPolarity;              // 极性，默认：MVD_POLARITY_TYPE_Bright
		MVD_RANGE stThreshold;                     // 阈值范围，默认：100，150
		int nSoftness;                             // 软阈值柔和度，默认：254
		MVD_RANGE stSoftThreshold;                 // 固定软阈值范围，默认：100，150
		MVD_RANGE stTailRatio;                     // 尾部比例范围，默认：5，5
		MVD_RANGE stThresholdRatio;                // 阈值比例范围，默认：40，60
		MVD_CONNECTIVITY_TYPE emConnectivity;      // 连通性，默认：MVD_CONNECTIVITY_TYPE_8
		int nMinHoleArea;                          // 孔洞最小面积，默认：0
		int nMaxFindNum;                           // 最大查找个数，默认：100
		MVD_RANGE_EX stArea;                       // 面积上下限，默认：10，999999999
		MVD_RANGE_EX stPerimeter;                  // 周长上下限，默认：10，999999999
		MVD_RANGE_EX stShortAxis;                  // 短轴上下限，默认：1，999999999
		MVD_RANGE_EX stLongAxis;                   // 长轴上下限，默认：10，999999999
		MVD_Range_ExF stAxisRatio;                 // 轴比上下限，默认：0.01，1.00
		MVD_Range_ExF stCircularity;               // 圆形度上下限，默认：0.10，1.00
		MVD_Range_ExF stRectangularity;            // 矩形度上下限，默认：0.10，1.00
		MVD_Range_ExF stCentroidBias;              // 质心偏移上下限，默认：0.00，9999999
		MVD_Range_ExF stBoxAngle;                  // 外接矩形角度上下限，默认：-90，90
		MVD_RANGE_EX stRectWidth;                  // 外接0度矩形宽上下限，默认：100，200
		MVD_RANGE_EX stRectHeight;                 // 外接0度矩形高上下限，默认：100，200
		MVD_FEATURE_TYPE emSortFeature;            // 排序特征，默认：MVD_FEATURE_TYPE_AREA
		MVD_SORT_TYPE emSortType;                  // 排序方式，默认：MVD_SORT_TYPE_DESC
		MVD_CONTOUR_TYPE emContourType;            // 输出轮廓类型，默认：MVD_CONTOUR_TYPE_NONE
		int nHistEnable;                           // 直方图，默认：0
		int nOutPixelEnable;                       // 超界像素过滤使能，默认：0
		int nOutPixelNum;                          // 超界像素值，默认：0
		int nReserved[256];                         // 保留字节

		// 优化后的默认构造函数 - 使用初始化列表
		_MVD_MULTI_ROI_BLOB_PARAM__() :
		emThresholdType(MVD_THRESHOLD_TYPE_SINGLE),
			emPolarity(MVD_POLARITY_TYPE_Bright),		
			nSoftness(254),
			emConnectivity(MVD_CONNECTIVITY_TYPE_8),
			nMinHoleArea(0),
			nMaxFindNum(100),
			emSortFeature(MVD_FEATURE_TYPE_AREA),
			emSortType(MVD_SORT_TYPE_DESC),
			emContourType(MVD_CONTOUR_TYPE_NONE),
			nHistEnable(0),
			nOutPixelEnable(0),
			nOutPixelNum(0)
		{

			stThreshold.nMin = 100;
			stThreshold.nMax = 150;

			stSoftThreshold.nMin = 100;
			stSoftThreshold.nMax = 150;

			stTailRatio.nMin = 5;
			stTailRatio.nMax = 5;

			stThresholdRatio.nMin = 40;
			stThresholdRatio.nMax = 60;


			stArea.bValid = true;
			stArea.nMin = 10;
			stArea.nMax = 999999999;

			stPerimeter.bValid = false;
			stPerimeter.nMin = 10;
			stPerimeter.nMax = 999999999;

			stShortAxis.bValid = false;
			stShortAxis.nMin = 1;
			stShortAxis.nMax = 999999999;

			stLongAxis.bValid = false;
			stLongAxis.nMin = 10;
			stLongAxis.nMax = 999999999;

			stAxisRatio.bValid = false;
			stAxisRatio.fMin = 0.01f;
			stAxisRatio.fMax = 1.0f;

			stCircularity.bValid = false;
			stCircularity.fMin = 0.1f;
			stCircularity.fMax = 1.0f;

			stRectangularity.bValid = false;
			stRectangularity.fMin = 0.1f;
			stRectangularity.fMax = 1.0f;

			stCentroidBias.bValid = false;
            stCentroidBias.fMin = 0.00f;
            stCentroidBias.fMax = 9999999;

			stBoxAngle.bValid = false;
			stBoxAngle.fMin = -90.0f;
			stBoxAngle.fMax = 90.0f;

			stRectWidth.bValid = false;
			stRectWidth.nMin = 100;
			stRectWidth.nMax = 200;

			stRectHeight.bValid = false;
			stRectHeight.nMin = 100;
			stRectHeight.nMax = 200;
		}
	};
	
	// 组合模块输出的缺陷信息类型
	typedef enum _MVD_FLAW_DETECT_COM_TYPE__
	{
		MVD_COMM_DET_OUT_FLAW_B  = 1, // 亮缺陷
		MVD_COMM_DET_OUT_FLAW_D  = 2, // 暗缺陷
		MVD_COMM_DET_OUT_FLAW_BD = 3, // 亮暗缺陷
	}MVD_FLAW_DETECT_COM_TYPE;


	/** @struct _MVD_MULTI_ROI_BLOB_PARAM__
	*  @brief  组合缺陷滤波、窗口二值化模块ROI的blob参数
	*/ 
	struct _MVD_FLAW_DETECT_COM_BLOB_PARAM__
	{
		// 查找参数
		MVD_CONNECTIVITY_TYPE  	emConnectivity;		  	// 连通性，默认：MVD_CONNECTIVITY_TYPE_8
		int					  	nMinHoleArea;			// 孔洞最小面积，默认：0
		int					  	nMaxFindNum;			// 最大查找个数，默认：100
		MVD_SORT_TYPE 		  	emSortType;				// 排序方式，默认：MVD_SORT_TYPE_ASC
		MVD_FEATURE_TYPE 	  	emSortFeature;      	// 排序特征，默认：MVD_FEATURE_TYPE_AREA

		// 输出使能
		bool 				  	bEnableHistInfo;   		// 直方图信息输出开关, 默认：True
		bool 				  	bEnableGrayInfoFlag;   	// 灰度信息输出开关, 默认：True
		bool  				  	bEnableContourInfoFlag;	// 轮廓信息输出开关, 默认：True
		...
		MVD_Range_ExF          	stCircularity;    		// 圆形度上下限，默认：0.10，1.00
		MVD_Range_ExF          	stRectangularity; 		// 矩形度上下限，默认：0.10，1.00
		MVD_RANGE_EX          	stCentroidBias;   		// 质心相对于矩形中心偏移上下限，默认：0，999999999
		MVD_Range_ExF          	stBoxAngle;       		// BOX角度上下限，默认：-90，90
		MVD_RANGE_EX           	stRectWidth;       		// rect_w上下限，默认：100，200
		MVD_RANGE_EX           	stRectHeight;      		// rect_h上下限，默认：100，200

		// 默认构造函数
		_MVD_FLAW_DETECT_COM_BLOB_PARAM__() :

			emConnectivity(MVD_CONNECTIVITY_TYPE_8),
			nMinHoleArea(0),
			nMaxFindNum(100),
			emSortFeature(MVD_FEATURE_TYPE_AREA),
			emSortType(MVD_SORT_TYPE_ASC),
			bEnableHistInfo(true),
			bEnableGrayInfoFlag(true),
			bEnableContourInfoFlag(true)
		{
			stArea.bValid = true;
			stArea.nMin = 10;
			stArea.nMax = 999999999;
			...
			stCentroidBias.bValid = false;
			stCentroidBias.nMin = 0;
			stCentroidBias.nMax = 999999999;
...
{
	bool bWarmupGpu;      // 开启GPU预热
	int  nReserved[8];    // 保留字节
}MVD_GLOBAL_INIT_PARAM;

//GPU锁频参数
typedef struct _MVD_GPU_CLOCKS_PARAM_
{
	int  nDeviceID;     // 显卡ID
	int  nMaxpercent;   // 设置为设备最大值的百分比，比如当前GPU最大主频为2000MHz，设为80表示设置为1600MHz 
	int  nReserved[8];  // 保留字节

}MVD_GPU_CLOCKS_PARAM;
